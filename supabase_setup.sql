...
-- This will create the company portal tables alongside your existing job seeker app
...
-- 8. Create storage bucket for company documents
INSERT INTO storage.buckets (id, name, public) 
VALUES ('company-documents', 'company-documents', true)
ON CONFLICT (id) DO NOTHING;

-- 9. Create storage policies for certificate uploads
DROP POLICY IF EXISTS "Companies can upload certificates" ON storage.objects;
CREATE POLICY "Companies can upload certificates" ON storage.objects
    FOR INSERT WITH CHECK (bucket_id = 'company-documents');

DROP POLICY IF EXISTS "Companies can view certificates" ON storage.objects;
CREATE POLICY "Companies can view certificates" ON storage.objects
    FOR SELECT USING (bucket_id = 'company-documents');

DROP POLICY IF EXISTS "Companies can update certificates" ON storage.objects;
CREATE POLICY "Companies can update certificates" ON storage.objects
    FOR UPDATE USING (bucket_id = 'company-documents');

-- 10. Create admin view for company verification
...
-- 12. Insert sample subscription plans data (optional)
-- You can uncomment these if you want to store plan info in the database
/*
...
('monthly', 'Monthly Plan', 'Post unlimited jobs for 30 days', 50.00, 30, '["Unlimited job postings", "Job management dashboard", "Email support", "30 days validity"]'),
('annual', 'Annual Plan', 'Post unlimited jobs for 365 days with significant savings', 500.00, 365, '["Unlimited job postings", "Job management dashboard", "Priority email support", "365 days validity", "Save $100 compared to monthly"]'),
('per_job', 'Per Job Plan', 'Pay only for the jobs you post', 30.00, null, '["Single job posting", "Job management dashboard", "Email support", "No expiry date"]')
ON CONFLICT (id) DO NOTHING;
*/
